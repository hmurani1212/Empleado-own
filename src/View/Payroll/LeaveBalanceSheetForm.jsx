...
const LeaveBalanceSheetForm = () => {
  ...
  const generateLeaveBalanceSheetReport = useStore((state) => state.generateLeaveBalanceSheetReport)
  ...
  const handleAutoFetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await generateLeaveBalanceSheetReport(requestData)
  ...
  }, [formData.branch_id, formData.department_id, formData.fromDate, formData.toDate, formData.selectedEmployee, generateLeaveBalanceSheetReport])
  ...
  const exportToExcel = (data, filename) => {
    try {
      ...
      const uniqueData = actualData.filter((item, index, self) => {
        const key = `${item.emp_id || item.employee_id}_${item.date || item.balance_date || 'no_date'}`
        return index === self.findIndex(t => `${t.emp_id || t.employee_id}_${t.date || t.balance_date || 'no_date'}` === key)
      })
      ...
      const excelData = uniqueData.map((item, index) => ({
        ...
        'Annual Leave': item.annual_leave || item.total_leave || '0',
        'Used Leave': item.used_leave || item.availed_leave || '0',
        'Remaining Leave': item.remaining_leave || item.balance_leave || '0'
      }))
  ...
  const exportToPDF = (data, filename) => {
    try {
      ...
      // Add title
      doc.setFontSize(16)
      doc.text('Leave Balance Sheet Report', 20, 20)
      ...
      doc.text('Father Name', 65, yPosition)
      doc.text('Designation', 100, yPosition)
      doc.text('Annual', 135, yPosition)
      doc.text('Used', 150, yPosition)
      doc.text('Remaining', 165, yPosition)
      ...
      uniqueData.forEach((item, index) => {
        ...
        doc.text(item.f_name || item.father_name || '-', 65, yPosition)
        doc.text(item.designation || '-', 100, yPosition)
        doc.text((item.annual_leave || item.total_leave || '0').toString(), 135, yPosition)
        doc.text((item.used_leave || item.availed_leave || '0').toString(), 150, yPosition)
        doc.text((item.remaining_leave || item.balance_leave || '0').toString(), 165, yPosition)
        yPosition += lineHeight
      })
            ...
            onClick={async () => {
              try {
                ...
                const employeeInfo = formData.selectedEmployee
                  ...
                  : '_All_Employees'
                const filename = `Leave_Balance_Sheet_Report_${timestamp}${employeeInfo}`
                ...
                if ((!reportData || reportData.length === 0) && formData.selectedEmployee) {
                  ...
                  setTimeout(() => {
                    if (!reportData || reportData.length === 0) {
                      showToast('No leave balance data found for the selected employee', 'warning')
                      return
                    }
                  }, 1000)
                  return
...
export default LeaveBalanceSheetForm
