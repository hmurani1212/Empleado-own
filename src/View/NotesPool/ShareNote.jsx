...
const BranchView = (props)=>{
    const {shareNoteValue, handleChangeShareNote} = props 
...
const DepartmentView = (props)=>{
    const {shareNoteValue, handleToggleSubDept, handleChangeShareNote} = props 
                ...
                {employeeCheckListValue?.departmentList?.departments?.length > 0 && employeeCheckListValue.departmentList?.departments?.map((ele) => (
                    <div key={ele.id}>
                        ...
                        <Checkbox
                            ...
                            value={ele.id}
                            onClick={() => handleToggleSubDept(ele.id)} // Toggle sub-departments
                            onChange={handleChangeShareNote}
                        />

                        {/* Sub-departments */}
                        {shareNoteValue.showSubDept.includes(ele.id) && ele.sub_dept && (
                            <div className='ml-4'>
...
const EmployeeView = (props)=>{
    const {shareNoteValue, handleSelectShareNote,handleChangeShareNote} = props
    const { fetchingAllBranches, empBranches } = useEmployees();
    const { employeeCheckListValue, handleEmpCheckList } = useEmployeeCheckList();
