...
/**
 * "Select Shared Notebook" picker (type 1) – inside the modal, no body portal.
 * Adds padding-bottom when open so the dialog body scrolls to show the full list.
 */
...
const BranchOnlyShareView = ({ handleChangeShareNote, shareValue }) => {
 ...
 * When showEmployeePickers is true, loads employees for Branch+Dept and shows checkboxes.
 */
...
const EmployeeView = (props) => (
    <ShareBranchDeptPair
        ...
        handleChangeShareNotebook={props.handleChangeShareNote}
        showEmployeePickers
        scopeKey={props.shareNoteValue?.shareWith}
    />
);
...
const ShareNote = (props) => {
    ...
    const { shareNoteValue, handleChangeShareNote, handleSelectShareNote , handleCopytoClipboard, handleCopytoClipboardMouseLeave, copied, handleShareNoteAdd, mySharedNotebooks} = props
    const loadingSharedList = shareNoteValue?.loadingMySharedNotebooks === true

    return (
        <div className='w-full max-w-full min-w-0 space-y-4 pb-10 sm:pb-12'>
            <div className='flex items-center justify-between'>
                {noteShareData.map((ele)=>(
                    <Radio 
                        key={ele.id}
                        name="type"
                        checked={ ele.id === shareNoteValue.type }
                        value={ele.id}
                        label={
                            <Typography>
                                {ele.title}
            ...
            <div className='px-3 pt-1 pb-2'>
                {
                        ...
                        <div className="space-y-2">
                            <div className='flex-1 px-0 sm:px-2 space-y-1 w-full min-w-0 max-w-full sm:max-w-md'>
                                ...
                                <input 
                                    ...
                                    type='text' 
                                    value={ shareNoteValue?.note_name}
                                    name='notebook_name' 
                                    onChange={handleChangeShareNote}
                                />
                            </div>
                                ...
                                <div className='flex flex-wrap'>
                                    {
                                        sharenotbookPermissionData.map((ele)=>(
                                            <Checkbox 
                                                ...
                                                checked={shareNoteValue.allowPermission.includes(ele.fieldName)}
                                                onChange={handleChangeShareNote}
                                            />
                                        ))
                                    }
                            ...
                            <div className='flex-1 px-2 space-y-1'>
                                ...
                                <div className='flex'>
                                    {
                                        sharenotebookShareWithData.map((ele)=>(
                                            <Checkbox 
                                                ...
                                                name="shareWith"
                                                value={ele.value}
                                                checked={ ele.value === shareNoteValue.shareWith }
                                                label={
                                                    <Typography className='text-[12px]'>
                                                        {ele.title}
                                                    </Typography>
                                ...
                                {shareNoteValue.shareWith === 'branch' ? (
                                    <BranchOnlyShareView
                                        shareValue={shareNoteValue}
                                        handleChangeShareNote={handleChangeShareNote}
                                    />
                                ) : shareNoteValue.shareWith === 'dept' ? (
                                    <ShareBranchDeptPair
                                        handleSelectShareNote={handleSelectShareNote}
                                        handleChangeShareNotebook={handleChangeShareNote}
                                        showEmployeePickers={false}
                                        scopeKey={shareNoteValue.shareWith}
                                    />
                                ) : shareNoteValue.shareWith === 'employee' ? (
                                    <EmployeeView
                    ...
                    shareNoteValue.type === 3 ? 

                    (
                        <div>
                            <Button
                                onMouseLeave={handleCopytoClipboardMouseLeave}
                                onClick={() => handleCopytoClipboard(shareNoteValue.textToCopy)}
            ...
            {shareNoteValue.type !== 3 && 
                <div className='px-3 pt-1 pb-3'>
                    <CustomButton 
                        title= 'Share'
                        onClick = {handleShareNoteAdd}
                        type="button"
                        loading = {shareNoteValue.loading}
                        />
                </div>
            }
        </div>
