...
export const formatAttendanceData = (apiResponse) => {
    ...
    const presentDaysFromArray = Array.isArray(data.attendance) 
        ? data.attendance.filter(record => record.status === 'present' || record.present === true || record.attendance_status === 'present').length 
        : 0
    const presentDays = data.present_days ?? (presentDaysFromArray || presentDaysFromHours || 0)
